...
def worker(model, config, states_path, out_path):
...
def main():
    ...
    ap.add_argument("--out", help="worker: output file; otherwise the results directory (default results/)")
    args = ap.parse_args()
    if args.worker:
        return worker(args.model, args.worker, args.states_file, args.out)

    res = args.out or os.path.join(ROOT, "results")
    ...
    for c in CONFIGS:  # one process per configuration, never two at once
        ...
        subprocess.run([sys.executable, __file__, "--worker", c, "--model", args.model, "--states-file", sp, "--out", op], check=True)
        out[c] = read_json(op)
    report = compare(args.model, states, out)
